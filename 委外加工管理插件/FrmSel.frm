...
Case 5
    existSql = "select FInterID from ICStockBill where FHeadSelfA0539='" + sysNo + "' and FCancellation = 0"
Case 6
    existSql = "select FInterID from ZPStockBill where FHeadSelfZIN36='" + sysNo + "' and FCancellation = 0"
Case 28
    existSql = "select FInterID from ICStockBill where FHeadSelfB0838='" + sysNo + "' and FCancellation = 0"
Case Else
End Select
...
If (existRs.RecordCount > 0) Then
    If MsgBox("该委外平台单号之前已经导进去过K3，是否继续？", vbYesNo, "信息中心提示") = vbNo Then
        existRs.Close
        Exit Sub
    End If
End If
...
If (TxtBillNo.Text = "") Then
    MsgBox "委外平台单号不可为空!", vbOKOnly, "信息中心提示"
    Exit Sub
End If
...
If TxtBillNo.Text <> "" Then
    StrWhere = "where 委外平台单号  = '" + TxtBillNo.Text + "' and FtranType =" + Str(trantype)
End If
    
StrSql = "select * from [192.168.100.210].[OutSourcing].dbo." + viewName + "  " + StrWhere + " order by 源单内码,源单分录 "
...
sql = "select field_en_name,head_or_entry from [192.168.100.210].[OutSourcing].[dbo].K3TableDes where trantype=" + Str(trantype) + " order by number"
...
Dim sql As String
Dim i As Integer
sql = "select field_cn_name,field_width from [192.168.100.210].[OutSourcing].[dbo].K3TableDes where trantype=" + Str(trantype) + " order by number"
Rs.Open sql, DBServer, adOpenKeyset
